...
func (c PopulationGreaterThan) IsMatch(feature *City) (bool, error) {
	return feature.Population > c.Threshold, nil
}
...
func Benchmark_Query(b *testing.B) {
	...
	bounds, _ := geo.NewBoundsAround(location, radius)
	for i := 0; i < b.N; i++ {
		index.Query(bounds, query.Build[CityID, *City]().Where(PopulationGreaterThan{500000}).Query())
	}
}
